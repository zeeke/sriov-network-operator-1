...
func validateRootDevices(current *sriovnetworkv1.SriovNetworkNodePolicy, previous *sriovnetworkv1.SriovNetworkNodePolicy) error {
	for _, curRootDevice := range current.Spec.NicSelector.RootDevices {
		curName, curRngSt, curRngEnd, err := sriovnetworkv1.ParseVfRange(curRootDevice)
		if err != nil {
			return fmt.Errorf("invalid rootDevice: %s", curRootDevice)
		}
		for _, preRootDevice := range previous.Spec.NicSelector.RootDevices {
			// Not validate return err of preRootDevice for previous PF
			// since it should already be evaluated in previous run.
			preName, preRngSt, preRngEnd, _ := sriovnetworkv1.ParseVfRange(preRootDevice)
			if curName == preName {
				err = validateExternallyManage(current, previous)
				if err != nil {
					return err
				...
				if curRngEnd < preRngSt || curRngSt > preRngEnd {
					...
					return fmt.Errorf("VF index range in %s is overlapped with existing policy %s", curRootDevice, previous.GetName())
				}
			}
		}
	}
