...
func tryEnableRdma() (bool, error) {
	...
	cmd := exec.Command("/bin/bash", filesystemRoot+rdmaScriptsPath)
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
...
func tryCreateSwitchdevUdevRule(nodeState *sriovnetworkv1.SriovNetworkNodeState) error {
	...
	filePath := filesystemRoot + "/host/etc/udev/rules.d/20-switchdev.rules"
	...
	var stdout, stderr bytes.Buffer
	cmd := exec.Command("/bin/bash", filesystemRoot+udevScriptsPath)
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return err
...
func tryCreateNMUdevRule() error {
	...
	dirPath := filesystemRoot + "/host/etc/udev/rules.d/"
